...
import numpy as np
import framework
from framework import cuda
from framework.core import Function, as_variable
from framework.utils import pair, get_conv_outsize, get_deconv_outsize
from framework.functions import linear, broadcast_to

# TODO(pycuda_demo)
# TODO(c++ cuda_demo)
# TODO(模型训练自动化调参模型)
# TODO(numpy 矢量化底层实现方法)
# =============================================================================
...
class Conv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        N, _, _, _, OH, OW = col.shape
        col = col.transpose((0, 4, 5, 1, 2, 3)).reshape((N * OH * OW, -1))
        t_W = W.reshape(W.shape[0], -1).T
        y = xp.dot(col, t_W)
        t_W = None
        y = y.reshape(N, OH, OW, Co)
...
def im2col_array(img, kernel_size, stride, pad, to_matrix=True):
    ...
    if xp != np:
        ...
        '''
        img = img.reshape(N * C, H, W)  # W,H ,N*C. if N*C H W will be better
        col = np.zeros((N * C * KH * KW, OH, OW), dtype=img.dtype)

        framework.pybind.im2col(img, col, KH, KW, SH, SW)  # OW, OH, N*C*KW*KH if N*C*KH*KW OH OW will be better
        col = col.reshape(N, C, KH, KW, OH, OW)  # n c kH kW oH oW
        '''
    if to_matrix:
        col = col.transpose((0, 4, 5, 1, 2, 3)).reshape((N * OH * OW, -1))
...
def col2im_array(col, img_shape, kernel_size, stride, pad, to_matrix=True):
    ...
    if xp != np:
        ...
        framework.pybind.col2im(img, col, KH, KW, SH, PH)
